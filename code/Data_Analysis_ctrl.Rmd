...
vd <- venn.diagram(list("case" = species, "control" = species_ctrl), 
                   ...
                   cat.col = "black", cat.cex = 1.5, fontface = "bold", 
                   filename = NULL)
grid.draw(vd)
...
```{r}
indiv <- log_commun_sick%*%pca_raw$rotation[,1:2]
plot(indiv[,1],indiv[,2],col=ifelse(clean_condition=='EN',"dodgerblue2","darkorange"),pch=16)
```
...
suppIndiv <- scale(log_commun_ctrl[which(condition_ctrl == "EN"),]
                   ,scale = FALSE)%*%res_plsda$loadings$X
points(suppIndiv[,1],suppIndiv[,2],pch=17)
```

```{r}
indiv=scale(log_commun_sick[sel_samples,],scale = FALSE)%*%res_plsda$loadings$X
