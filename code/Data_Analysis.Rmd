...
#l'espece c'est le dernier mot de chaque ligne avec les bacteries
```

The length of `unique(species)` is not the same as `unique(list_bacteria)`... maybe because the last not unknown name selected of one bacteria can be the same as a known species name ?
...
```{r }
list_duplicates<-unique(names(which(table(df_abundances[ ,1]) > 1)))
duplicates_merged<-as.data.frame(t(sapply(list_duplicates,FUN=function(x){as.data.frame(t(colSums(abundances[grep(x,df_abundances[,1]),])))})))
list_pas_duplicates <- unique(names(which(table(df_abundances[ ,1]) == 1)))
clean_data <- rbind(abundances[match(list_pas_duplicates,df_abundances[,1]),],duplicates_merged)
```

`clean_data` does not contain any duplicate: duplicates have been added together.

```{r}
rownames(clean_data)<-c(list_pas_duplicates,list_duplicates)
colnames(clean_data) <- id_abundances
list_bacteria<-rownames(clean_data)   # we keep the bacteria names
clean_data <- clean_data[,-grep("29",id_abundances)]
colnames(clean_data) <- paste0("sample",id_abundances[-grep("29",id_abundances)],"_",condition[-grep("29",id_abundances)])
dim(clean_data)
```
We now have `r ncol(clean_data)` samples and `r nrow(clean_data)` bacteria.
...
```{r distBefore}
abundances <- as.data.frame(sapply(clean_data,as.numeric))
df<-abundances
...
The list of calve names is not in the same order in the `pathogenes` file as in the `abundances` file. We decided to keep the  `abundances` file order:
...
```{r }
fit <- randomForest(Y$Ct.RSV ~ ., data = X,classwt=c(1E-3,1E3),ntree=3000,mtry=10)
fit$confusion
```
Nothing in class `1`...
```{r }
...
```{r }
fit <- randomForest(Y$Ct.PI.3 ~ ., data = X,classwt=c(1E-2,1E3),ntree=1000,mtry=10)
fit$confusion
```
Nothing in class `1`...
```{r }
...
```{r }
fit <- randomForest(Y$Ct.Coronavirus ~ .,data = X,ntree=2000,mtry=16)
fit$confusion
```
High error
```{r }
...
```{r }
fit <- randomForest(Y$Ct.P.multocida ~ ., data = X,classwt=c(1E2,1E-3),ntree=1000,mtry=16)
fit$confusion
```
Nothing in class `0`
```{r }
...
```{r }
fit <- randomForest(Y$Ct.M.haemolytica ~ ., data = X,ntree=1000,mtry=40)
fit$confusion
```
...
```{r }
fit <- randomForest(Y$Ct.M.bovis ~ ., data = X,classwt=c(1E-3,1E3) , ntree=500,mtry=40)
fit$confusion
```
...
```{r }
fit <- randomForest(Y$Ct.H.somni ~ ., data = X,ntree=1000,mtry=16)
fit$confusion
```
...
plotVar(res_splsda)
```
Si on limite le nombre de variables, on n'arrive pas à discrimner de manière linéaire la présence ou l'absence du virus Coronavirus.
