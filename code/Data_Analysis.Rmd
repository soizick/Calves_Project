...
The length of unique species is not the same as unique bacteria...
...
df <- data.frame(t(abundances_ILR))  # on transpose pour avoir une matrice 273*45 comme les autres
names(df) <- paste0("sample", 1:ncol(df))
dim(df)
ggplot(df, aes(x = sample1)) + geom_histogram(bins = 50) + theme_bw() + 
  xlab("counts (sample 1)")
...
```{r }
fit <- randomForest(Y$Ct.Coronavirus ~ .,data = X,classwt=table(Y$Ct.Coronavirus),ntree=2000,mtry=16)
fit$confusion
```
High error
```{r }
