...
```{r }
fit <- randomForest(Y$Ct.RSV ~ ., data = X,ntree=5000,mtry=40)
fit$confusion
```
Nothing in class `1`...
```{r }
...
```{r }
fit <- randomForest(Y$Ct.PI.3 ~ ., data = X,ntree=1000,mtry=10)
fit$confusion
```
Nothing in class `1`...
```{r }
...
```{r }
fit <- randomForest(Y$Ct.Coronavirus ~ ., data = X,ntree=1000,mtry=16)
fit$confusion
```
High error
```{r }
...
```{r }
fit <- randomForest(Y$Ct.P.multocida ~ ., data = X,ntree=1000,mtry=10)
fit$confusion
```
Nothing in class `0`
```{r }
...
```{r }
fit <- randomForest(Y$Ct.M.bovis ~ ., data = X,ntree=500,mtry=40)
fit$confusion
```
