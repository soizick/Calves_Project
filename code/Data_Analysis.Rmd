...
# sPLS with regression mode

```{r}
pls <- pls(as.matrix(sapply(X,as.numeric)), as.matrix(sapply(Y,as.numeric)), ncomp = 8, mode = "regression")
spls <- spls(as.matrix(sapply(X,as.numeric)), as.matrix(sapply(Y,as.numeric)), ncomp =8, 
             keepX= c(15,15), keepY=c(7,7), multilevel=as.matrix(clean_id), mode = "regression")
```

```{r}
tune.pls <- perf(pls, validation = "Mfold", folds = 20, multilevel=as.matrix(clean_id), 
                 progressBar = FALSE, nrepeat = 10)
tune.spls <- perf(spls, validation = "Mfold", folds = 20, multilevel=as.matrix(clean_id), 
                  progressBar = FALSE, nrepeat = 10)
```
...
```
We have to select the number of components if its value is above 0.0975. But here, none of the component is above this value...

## Sample Plots

```{r}
plotIndiv(spls, comp = 1:2, rep.space= 'Y-variate', group = clean_condition, ind.names = clean_id,
legend = TRUE, title = 'sPLS comp 1 - 2, Y-space')
```
```{r}
plotIndiv(spls, comp = 1:2, rep.space= 'X-variate', group = clean_condition, ind.names = clean_id,
legend = TRUE, title = 'sPLS comp 1 - 2, X-space')
```
```{r}
plotIndiv(spls, comp = 1:2, rep.space= 'XY-variate', group = clean_condition, ind.names = clean_id,
